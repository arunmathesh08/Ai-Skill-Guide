    ...
    <div className={`w-full ${className}`}>
      {showLabel && (
        <div className="flex justify-between items-center text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">
          ...
          <span className="font-mono text-slate-900 dark:text-slate-100">{percentage}%</span>
        </div>
      )}
      <div className={`relative w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden ${heightClasses[height]}`}>
        <div
        ...
        {targetPercentage !== null && (
          <div
            className="absolute top-0 bottom-0 w-0.5 bg-slate-900 dark:bg-slate-200 z-10 opacity-70"
